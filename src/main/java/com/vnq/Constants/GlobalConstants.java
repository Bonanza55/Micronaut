package com.vnq.Constants;

public class GlobalConstants {
    public final static String DB_OPERATION_ERROR = "***** DB-OPERATION-ERROR *****";
    public final static String DB_OPERATION_SUCCESS = "***** DB-OPERATION-SUCCESS *****";
    public final static String OUT_FILE_ERROR = "***** OPEN OUTPUT FILE ERROR *****";
    public final static String REPORT_RUN_SUCCESS = "***** REPORT RUN SUCCESS *****";
    public final static String JSON_PARSE_ERROR = "***** JSON PARSE ERROR *****";
    public final static int UPDATE_FAIL = -100;
    public final static int  INSERT_FAIL = -100;
}
